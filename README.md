...
VerifAI is a production-grade, multi-agent Retrieval-Augmented Generation (RAG) pipeline designed to automate the medical, legal, and regulatory (MLR) compliance verification of clinical and pharmaceutical claims. It parses marketing copy, structures clinical PICOT boundaries, performs compliance-aware hybrid retrieval from a secure vector store, and employs a rigorous multi-agent judge framework to guarantee **0% hallucination rates** through verbatim anchor validation.
...
The following sequence details how an incoming clinical claim is processed through the system:

```
[Claim Text Input]
      │
      ▼
1. Classify (GPT-5.5) ──────► Output: CT-ID (e.g. CT-201 Efficacy) + PICOT Components
      │
      ▼
2. Pre-Filter Tiers ────────► Allowed RT-IDs (Exclude Tier N, Boost P/A/C)
      │
      ▼
3. Rewrite Query ───────────► Clinical Question (Claude, e.g. "What was the INCAT improvement?")
      │
      ▼
4. Generate Embedding ──────► ncbi/MedCPT-Query-Encoder (768-dim vector)
      │
      ▼
5. Hybrid Retrieval ────────► Qdrant Dense (0.7 weight) + Keyword (0.3 weight)
                            Ranked with RRF Fusion & Tier Boosting (P: x2, A: x1, C: x0.5)
      │
      ▼
6. Compliance Judge ────────► Claude 3.5 Sonnet validates passages against 8 criteria
                            Mandates exact numeric match & verbatim anchor strings
      │
      ▼
7. Logic Gate Verdict ──────► Deterministic Verdict Map: PASS / SOFT_FLAG / BLOCK
                            Average Coverage Score, Audit Trail Logger
```
