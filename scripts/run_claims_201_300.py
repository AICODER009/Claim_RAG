#!/usr/bin/env python3
"""Run the NEXT 100 unique claims (201-300) through full substantiation pipeline → MD report.

Skips first 200 unique claims (already tested), processes the next 100.
3-signal retrieval: Dense(MedCPT) + BM25(fastembed) + AND-match keywords → Claude judge.
"""
...
SKIP = 200        # Skip first 200 unique claims (already done)
LIMIT = 100       # Process next 100
BATCH_LABEL = "201–300"
GLOBAL_OFFSET = 200   # Global index starts at 201

OUTPUT_MD = Path(__file__).parent.parent / "claims" / "claims_201_300_results.md"


def load_claims_range(cfg, skip=SKIP, limit=LIMIT):
    ...
    if claims:
        ...
        logger.warning(f"Skipped {skipped} claims but found 0 new claims!")
    return claims
...
def main():
    ...
    claims = load_claims_range(cfg, skip=SKIP, limit=LIMIT)
    logger.info(f"Loaded {len(claims)} claims for batch {BATCH_LABEL}")

    if not claims:
        print("ERROR: No claims loaded. Check skip/limit values.")
        return
    ...
    q_model = AutoModel.from_pretrained(
        "ncbi/MedCPT-Query-Encoder", cache_dir=r"D:\hf_cache", low_cpu_mem_usage=True
    )
    q_model.eval()
    q_model.half()   # fp16 to save RAM
    logger.info("MedCPT ready (float16)")

    # Load BM25 model (lightweight CPU tokenizer)
    ...
    # Claim rewriter — use Anthropic (OpenAI quota exhausted)
    ...
    # Hybrid retriever: dense + BM25 sparse + AND-match keywords → RRF → cross-encoder → tier boost
    ...
    for i, claim_data in enumerate(claims, 1):
        ...
        try:
            # Step 1: Rewrite claim → focused retrieval question
            question = rewriter.rewrite(claim_text)

            # Step 2: Encode + 3-signal retrieve
            #   Signal 1: Dense (MedCPT cosine) — semantic understanding
            #   Signal 2: BM25 sparse (fastembed) — lexical matching with stemming
            #   Signal 3: AND-match keywords — exact phrase matching
            #   → RRF fusion → cross-encoder rerank → tier boost
            query_vec = encode_query(question)
            passages = retriever.search(
                query_vector=query_vec,
                query_text=question,
                bm25_query_text=claim_text,   # Original claim for BM25 (preserves exact terms)
                ct_id=ct_id,
                final_top_k=25,               # Raised 20→25 to give diversity cap room
            )
        ...
        # Checkpoint every 25 claims
        if i % 25 == 0:
            _write_report(results, OUTPUT_MD, BATCH_LABEL, partial=True)
            logger.info(f"  Checkpoint: {i}/{total} claims saved")

    total_time = time.time() - start
    _write_report(results, OUTPUT_MD, BATCH_LABEL, total_time=total_time)

    vc = Counter(r["verdict"] for r in results)
    coverages = [r.get("coverage", 0) for r in results if isinstance(r.get("coverage"), (int, float))]
    avg_cov = sum(coverages) / len(coverages) if coverages else 0

    print(f"\n{'='*60}")
    print(f"COMPLETE — Batch {BATCH_LABEL}: {len(results)} claims in {total_time:.0f}s ({total_time/60:.1f} min)")
    print(f"  ✅ PASS:      {vc.get('PASS', 0)}")
    print(f"  ⚠️  SOFT_FLAG: {vc.get('SOFT_FLAG', 0)}")
    print(f"  ❌ BLOCK:     {vc.get('BLOCK', 0)}")
    print(f"  ❓ ERROR:     {vc.get('ERROR', 0)}")
    print(f"  Avg coverage: {avg_cov:.0f}%")
    print(f"  Report: {OUTPUT_MD}")
    print(f"{'='*60}")


def _write_report(results, output_path, batch_label, partial=False, total_time=None):
    """Write MD report — callable both mid-run (checkpoint) and at end."""
    vc = Counter(r["verdict"] for r in results)
    coverages = [r.get("coverage", 0) for r in results if isinstance(r.get("coverage"), (int, float))]
    avg_cov = sum(coverages) / len(coverages) if coverages else 0

    md = []
    title = f"Claims {batch_label} — Substantiation Results"
    if partial:
        title += f" (IN PROGRESS — {len(results)} so far)"
    md.append(f"# {title}\n")
    md.append(f"**Date:** {time.strftime('%Y-%m-%d %H:%M')}")
    if total_time:
        md.append(f"**Total time:** {total_time:.0f}s ({total_time/60:.1f} min)")
        md.append(f"**Average per claim:** {total_time/max(len(results),1):.1f}s\n")
    else:
        md.append("")
    ...
    for r in results:
        ...
        if r.get("top_passage_ref"):
            md.append(
                f"- **Top match:** `{r.get('top_passage_rt_id', '?')}` "
                f"(tier: {r.get('top_passage_tier', '?')}) "
                f"from `{r.get('top_passage_ref', '?')[:55]}` "
                f"(score: {r.get('top_passage_score', 0)})\n"
            )
