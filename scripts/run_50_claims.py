#!/usr/bin/env python3
"""Run first 50 unique claims through full substantiation pipeline → MD report."""
...
OUTPUT_MD = Path(__file__).parent.parent / "claims" / "first_50_results.md"


def load_unique_claims(cfg, limit=50):
    """Load first N unique claim texts with CT-IDs."""
...
def main():
    ...
    claims = load_unique_claims(cfg, limit=50)
    logger.info(f"Loaded {len(claims)} unique claims")
    ...
    # Load BM25 model (fastembed Qdrant/bm25 — lightweight CPU tokenizer)
    ...
    judge = SubstantiationJudge(
        ...
        model="claude-sonnet-4-6",  # Sonnet 4.6 — better at scanning long evidence passages
        requirements_path=cfg.substantiation_requirements_path,
    )

    # Init query encoder — force GC first to reclaim memory
    import gc
    gc.collect()
    ...
    q_model.eval()
    q_model.half()  # Convert to float16 AFTER loading to reduce memory
    ...
    for i, claim_data in enumerate(claims, 1):
        ...
        try:
            ...
            # Step 2: Encode + Retrieve
            # Dense: rewritten query → MedCPT
            # BM25:  original claim text → fastembed sparse (preserves exact terms)
            query_vec = encode_query(question)
            passages = retriever.search(
                query_vector=query_vec,
                query_text=question,
                bm25_query_text=claim_text,  # BM25 uses original claim text
                ct_id=ct_id,
                final_top_k=20,
            )
            ...
            result = {
                ...
                # All passages sent to judge (for debugging BLOCKs)
                "passage_summaries": [
                    {
                        "rank": j + 1,
            ...
            logger.error(f"  [{i}/50] ERROR: {e}")

    total_time = time.time() - start

    # ── Build MD Report ──
    md = []
    md.append("# First 50 Claims — Substantiation Results\n")
